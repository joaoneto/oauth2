...
var buildUri = function (uri, params) {
  var result = [];
  var parse_url = utils.parseUri(uri);
  
  if (parse_url.protocol)
    result.push([parse_url.protocol, '://'].join(''));

  if (parse_url.userInfo)
    result.push([parse_url.userInfo, '@'].join(''));

  if (parse_url.host)
    result.push(parse_url.host);

  if (parse_url.port)
    result.push([':', parse_url.port].join(''));

  if (parse_url.path)
    result.push(parse_url.path);

  if (parse_url.query)
    result.push(['?', parse_url.query].join(''));

  if (parse_url.anchor)
    result.push(['#', parse_url.anchor].join(''));
...
OAuth2.handleAuthorization = function () {
  // {"client_id": "513ddd8af88071fb1c000003", "redirect_uri": "http://localhost:3000"}
  return function (req, res) {
    var result = {};
    ...
    res.send(buildUri(req.body.redirect_uri));
