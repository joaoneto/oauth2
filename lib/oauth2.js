...
var errorWWWAuthenticateResponseHeader = function (res, status_code, error, realm, scope, next) {
  function quote(str) {
    return "'" + str + "'";
  };
...
OAuth2.verifyAccessToken = function (scope, realm) {
  ...
  return function (req, res, next) {
    ...
    if (header_params.scheme === 'Invalid')
      errorWWWAuthenticateResponseHeader(res, 400, 'invalid_request', realm, scope, next);

    self.getAccessToken(oauth_token, function (err, token) {
      if (!token)
        errorWWWAuthenticateResponseHeader(res, 401, 'invalid_request', realm, scope, next);

      if (!token.expires || new Date().getTime() > token.expires)
        errorWWWAuthenticateResponseHeader(res, 401, 'expired_token', realm, scope, next);
