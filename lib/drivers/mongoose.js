...
OAuth2.getAuthCode = function (code, next) {
  AuthorizationCode.findOne({code: code}, function (err, auth_code) {
    if (err)
      return next(500, 'internal_service_error');
...
OAuth2.setAccessToken = function (access_token_config, next) {
  ...
  var access_token = new AccessToken(access_token_config);
...
OAuth2.getAccessToken = function (oauth_token, next) {
  AccessToken.findOne({oauth_token: oauth_token}, function (err, token) {
    return next(null, token);
  });
};